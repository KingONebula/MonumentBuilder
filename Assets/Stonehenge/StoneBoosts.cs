...
public class StoneBoosts : MonoBehaviour
{
    ...
    {
        if (boostTime > 0)
            gameManager.stonehengeBoost = 2;
        else
            gameManager.stonehengeBoost = 1;
        if (gameManager.stonehenge >= 2)
        {
            ...
            if (hit.collider != null && canPlaygame && hit.collider.tag == "Stone" && Input.GetKeyDown(KeyCode.Mouse0))
            {
